...
class ShallowGibbsPolicy(DiscretePolicy):
    ...
    def stabilizer(self, s):
        with torch.no_grad():
            s = torch.clamp(torch.tensor(s), 0, self.n_states - 1)
            s = tu.complete_in(s, 1)
            actions = torch.arange(self.n_actions).repeat(s.shape + (1,)) #|s| x n_a
            states = s.repeat((self.n_actions,) + (1,)*len(s.shape)).transpose(0,1) #|s| x n_a
            prefs = self.pref(self.feat(states, actions)).squeeze(-1) #|s| x n_a
            return torch.max(prefs, -1)[0] #|s|
    ...
    def act(self, s, deterministic=False):
        with torch.no_grad():
            actions = torch.arange(0, self.n_actions).repeat(s.shape + (1,)) #|s| x n_a
            states = s.repeat(self.n_actions, 1).t() #|s| x n_a
    ...
    def score(self, s, a):
        with torch.no_grad():
            ...
            return 1. / self.temp * (self.feat(s, a) - expect)
