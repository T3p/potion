...
for param in params:
    ...
    fo.append(power_fo(pol, disc, env, std, horizon=horizon))
    op.append(power_op(pol, std, disc, horizon, minibatchsize=10, env=env))
    estimated.append(power(pol, batch, grad, disc, clip=1.2))
...
plt.plot(params, fo, label='FO')
plt.plot(params, op, label='OP')
